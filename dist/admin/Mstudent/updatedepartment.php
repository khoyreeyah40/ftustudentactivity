...
if (isset($_GET['update_id']) && !empty($_GET['update_id'])) {
    $id = $_GET['update_id'];
    $stmt_update = $DB_con->prepare('SELECT mainorg.*, department.*, faculty.* FROM department
    JOIN faculty ON faculty.faculty = department.dpmfct
    JOIN mainorg ON mainorg.mainorgNo = faculty.faculty
        WHERE department.dpmNo = :dpmNo');
    $stmt_update->execute(array(':dpmNo' => $id));
    $update_row = $stmt_update->fetch(PDO::FETCH_ASSOC);
    extract($update_row);
} else {
    header("Location: department.php");
}
if (isset($_POST['btupdatedpm'])) {
    $dpmNo = $_POST['dpmNo'];
    $department = $_POST['department'];
    $dpmfct = $_POST['dpmfct']; // user name
...
<head>
    ...
    <title>ระบบกิจกรรมนักศึกษา| แก้ไขรายชื่อสาขา</title>
...
<body class="fixed-navbar">
    <div class="page-content fade-in-up" style="padding:20px;padding-top:0px">
        <div class="page-heading">
            <h1 class="page-title">จัดการรายชื่อสาขา</h1>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                </li>
                <li class="breadcrumb-item"><a href="department.php">จัดการรายชื่อสาขา</a></li>
                <li class="breadcrumb-item">แก้ไขรายชื่อสาขา</li>
            </ol>
        </div>
        <br>
        <br>
            ...
            <div class="ibox-body">
                <form class="form-horizontal" id="form-sample-1" method="post" novalidate="novalidate" style="height:150px; width:100%">
                    <div class="form-group row">
                        ...
                        <option value="<?php echo $dpmfct ?>"> <?php echo $mainorg ?></option>
                            <?php
                            ...
                            <input class="form-control" type="hidden" name="dpmNo" value="<?php echo $dpmNo; ?>" required />

                    <div class="form-group row">
                        <div class="col-sm-12 text-center">
                            <button class="btn btn-warning" type="submit" name="btupdatedpm">แก้ไข</button>
                            <a href="department.php"><button class="btn btn-danger" type="button">ยกเลิก</button></a>
                        </div>
                    </div>
                </form>
            </div>
